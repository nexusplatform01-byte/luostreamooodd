...
export default function LoginModal() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', background: '#181818', borderRadius: 12, width: 340, maxWidth: '94vw', padding: '22px 22px 18px', boxShadow: '0 20px 60px rgba(0,0,0,0.85)', border: '1px solid rgba(255,255,255,0.07)' }}>
        <button onClick={closeLogin} style={{ position: 'absolute', top: 12, right: 12, background: 'none', border: 'none', color: '#555', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 24, height: 24 }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
        </button>

        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <div style={{ fontFamily: 'Arial Black, Arial, sans-serif', fontSize: 17, fontWeight: 900, letterSpacing: 2 }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', marginBottom: 16 }}>
          {(['login', 'register'] as const).map(t => (
            <button key={t} onClick={() => { setTab(t); setError(''); }}
              style={{ flex: 1, background: 'none', border: 'none', color: tab === t ? '#e50914' : '#666', fontFamily: 'Arial, sans-serif', fontWeight: 700, fontSize: 11, letterSpacing: 1.5, padding: '8px 0', cursor: 'pointer', borderBottom: tab === t ? '2px solid #e50914' : '2px solid transparent', marginBottom: -1 }}>
        ...
        {error && (
          <div style={{ background: 'rgba(229,9,20,0.1)', border: '1px solid rgba(229,9,20,0.25)', borderRadius: 6, padding: '8px 10px', marginBottom: 12, color: '#e50914', fontSize: 10, fontFamily: 'Arial, sans-serif', letterSpacing: 0.8 }}>
            {error}
          </div>
        )}
        ...
        <button onClick={tab === 'login' ? handleLogin : handleRegister} disabled={loading}
          style={{ width: '100%', marginTop: 14, padding: '11px', background: loading ? '#444' : 'linear-gradient(135deg,#e50914,#c0000a)', border: 'none', borderRadius: 8, color: '#fff', fontFamily: 'Arial, sans-serif', fontWeight: 700, fontSize: 11, letterSpacing: 2, cursor: loading ? 'not-allowed' : 'pointer' }}>
          {loading ? 'PLEASE WAIT...' : (tab === 'login' ? 'LOG IN' : 'CREATE ACCOUNT')}
        </button>

        <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '12px 0' }}>
          <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.07)' }} />
          <span style={{ color: '#444', fontSize: 10, fontFamily: 'Arial, sans-serif', letterSpacing: 1 }}>OR</span>
          <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.07)' }} />
        </div>

        <button onClick={handleGoogle} disabled={loading}
          style={{ width: '100%', padding: '10px', background: '#fff', border: 'none', borderRadius: 8, color: '#222', fontFamily: 'Arial, sans-serif', fontWeight: 700, fontSize: 11, letterSpacing: 1, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        {tab === 'login' ? (
          <p style={{ textAlign: 'center', marginTop: 12, color: '#555', fontSize: 10, fontFamily: 'Arial, sans-serif', letterSpacing: 0.8 }}>
            NO ACCOUNT?{' '}
...
const inp: React.CSSProperties = { width: '100%', padding: '10px 13px', background: '#242424', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, color: '#fff', fontFamily: 'Arial, sans-serif', fontSize: 11, letterSpacing: 1, outline: 'none', boxSizing: 'border-box' };
